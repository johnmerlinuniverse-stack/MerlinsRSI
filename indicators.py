...
# ============================================================
# SMART MONEY FLOW (based on BOSWaves CLV methodology)
# ============================================================
...
def calculate_smart_money_flow(
    ...
    """
    Smart Money Flow — measures accumulation/distribution by analyzing
    WHERE price closes within each bar, weighted by volume.

    Logic (adapted from BOSWaves Smart Money Flow Cloud):
    1. CLV (Close Location Value) = ((close - low) - (high - close)) / (high - low)
       → +1 if close at high (buyers dominate), -1 if close at low (sellers dominate)
    2. Raw Flow = CLV × volume  (volume-weighted directional pressure)
    3. Money Flow Ratio = sum(raw_flow, N) / sum(|raw_flow|, N)  → net direction [-1, +1]
    4. Smoothed MF = EMA(MF_Ratio, smooth_period)
    5. Flow Strength = |smoothed_mf| ^ power  → nonlinear emphasis [0, 1]

    Returns:
        smf_value: float    — smoothed money flow ratio (-1.0 to +1.0)
        smf_strength: float — flow strength (0.0 to 1.0)
        smf_trend: str      — BULLISH / BEARISH / NEUTRAL
        smf_signal: str     — STRONG_ACCUMULATION / ACCUMULATION / DISTRIBUTION / STRONG_DISTRIBUTION / NEUTRAL
        smf_diverging: bool — True if flow direction opposes price direction (smart money divergence)
    """
    default = {
        "smf_value": 0.0, "smf_strength": 0.0,
        "smf_trend": "NEUTRAL", "smf_signal": "NEUTRAL",
        "smf_diverging": False,
    }
    ...
    try:
        ...
        # Numerator: sum of signed flow (net direction)
        # Denominator: sum of |flow| (total activity)
        ...
        # (e.g., price falling but smart money accumulating)
...
def generate_confluence_signal(
    ...
    # ── Smart Money Flow scoring (±18 max) ──
    if smf_data:
        smf_val = smf_data.get("smf_value", 0)
        smf_sig = smf_data.get("smf_signal", "NEUTRAL")
        smf_div = smf_data.get("smf_diverging", False)
        if smf_sig == "STRONG_ACCUMULATION":
            score += 18; reasons.append(f"SMF strong accumulation ({smf_val:+.2f})")
        elif smf_sig == "ACCUMULATION":
            score += 10; reasons.append(f"SMF accumulation ({smf_val:+.2f})")
        elif smf_sig == "STRONG_DISTRIBUTION":
            score -= 18; reasons.append(f"SMF strong distribution ({smf_val:+.2f})")
        elif smf_sig == "DISTRIBUTION":
            score -= 10; reasons.append(f"SMF distribution ({smf_val:+.2f})")
        # Divergence bonus: smart money opposes price → extra weight
        if smf_div:
            if smf_val > 0:
                score += 7; reasons.append("SMF bullish divergence ⚡")
            else:
                score -= 7; reasons.append("SMF bearish divergence ⚡")
