...
# ============================================================
# SMART MONEY FLOW (CLV × Volume — based on BOSWaves methodology)
# ============================================================
...
def calculate_smart_money_flow(
    ...
    1. CLV = ((close-low) - (high-close)) / (high-low) → +1 at high, -1 at low
    2. Raw Flow = CLV × volume
...
def compute_individual_scores(
    ...
    # --- Smart Money Flow (max ±18, +7 divergence bonus = ±25 total) ---
    s = 0; r = ""
    if smf_data:
        ...
        if smf_sig == "STRONG_ACCUMULATION":
            s = 18; r = f"SMF starke Akkumulation ({smf_val:+.2f})"
        elif smf_sig == "ACCUMULATION":
            s = 10; r = f"SMF Akkumulation ({smf_val:+.2f})"
    ...
    max_weights = {
        ...
        "fear_greed": 8, "nr_breakout": 18, "smf": 25,
    }
...
def compute_alert_priority(individual: dict, signal: str) -> dict:
    ...
    if scores.get("rsi_divergence", 0) != 0:
        ...
        if div_aligned:
            special.append("DIV")
    if scores.get("smf", 0) != 0:
        smf_aligned = (is_bullish and scores["smf"] > 0) or (not is_bullish and scores["smf"] < 0)
        if smf_aligned and abs(scores["smf"]) >= 18:
            special.append("SMF")
    ...
    for offset in range(forward + 1, forward + lookback + 1):
        ...
        try:
            ...
            individual = compute_individual_scores(
                ...
                smf_data=calculate_smart_money_flow(slice_df) if len(slice_df) >= 34 else None,
            )
            scan_filters = {"rsi_4h": True, "rsi_1d": False, "macd": True, "volume_obv": True,
                           ...
                           "funding_rate": False, "fear_greed": False, "smf": True}
...
def generate_confluence_chart(df: pd.DataFrame, symbol: str, active_filters: dict, score: int = 0, rec: str = "") -> bytes:
    ...
    filter_names = {"rsi_4h": "RSI", "rsi_1d": "RSI 1D", "macd": "MACD", "volume_obv": "Volume+OBV",
                    ...
                    "funding_rate": "Funding", "fear_greed": "F&G", "smf": "SMF"}
