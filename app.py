...
def scan_all(coins, tfs, smc=False):
    ...
    for sym in coin_list:
        ...
        if ptf and ptf in kld:
            ...
            smf_d=calculate_smart_money_flow(kld[ptf]); r["smf_value"]=smf_d["smf_value"]; r["smf_strength"]=smf_d["smf_strength"]; r["smf_trend"]=smf_d["smf_trend"]; r["smf_signal"]=smf_d["smf_signal"]; r["smf_diverging"]=smf_d["smf_diverging"]
            if smc:
            ...
            cf=generate_confluence_signal(rsi_4h=r.get("rsi_4h",50),rsi_1d=r.get("rsi_1D",50),macd_data=md,volume_data=vd,smc_data=sc_d,smf_data=smf_d)
            r["score"]=cf["score"]; r["confluence_rec"]=cf["recommendation"]; r["reasons"]=" | ".join(cf["reasons"][:3])
        else:
            r.update({"macd_trend":"NEUTRAL","macd_histogram":0,"vol_trend":"—","vol_ratio":1.0,"obv_trend":"—","stoch_rsi_k":50.0,"stoch_rsi_d":50.0,"smf_value":0.0,"smf_strength":0.0,"smf_trend":"NEUTRAL","smf_signal":"NEUTRAL","smf_diverging":False,"score":0,"confluence_rec":"WAIT","reasons":""})
        results.append(r)
...
- **Smart Money Flow** (max ±25 Punkte): CLV×Volume Akkumulation/Distribution + Divergenz-Bonus. Misst WO der Preis innerhalb jeder Kerze schließt, gewichtet mit Volumen
...
4. Setze SL/TP basierend auf den Empfehlungen im Risk Management""")
    ...
    for _,r in df[df["score"]>0].sort_values("score",ascending=False).head(15).iterrows():
        s=r["score"]
        smf_icon = "🟢" if r.get("smf_trend")=="BULLISH" else ("🔴" if r.get("smf_trend")=="BEARISH" else "⚪")
        smf_txt = f' | SMF: {smf_icon}{r.get("smf_value",0):+.2f}' if r.get("smf_value",0) != 0 else ""
        div_txt = " ⚡DIV" if r.get("smf_diverging") else ""
        st.markdown(f'<div style="background:#1a1a2e;border-radius:8px;padding:10px 14px;margin:4px 0;"><div style="display:flex;justify-content:space-between;"><b style="color:white;">{r["symbol"]}</b><span style="color:#00FF7F;font-weight:bold;">{r.get("confluence_rec","WAIT")} ({s})</span></div><div style="background:#2a2a4a;border-radius:4px;height:6px;margin-top:6px;"><div style="background:linear-gradient(90deg,#00FF7F,#32CD32);height:6px;width:{min(abs(s),100)}%;border-radius:4px;"></div></div><div style="font-size:11px;color:#888;margin-top:4px;">RSI 4h: {r.get("rsi_4h",50):.1f} | MACD: {r.get("macd_trend","—")} | Vol: {r.get("vol_trend","—")}{smf_txt}{div_txt}</div></div>', unsafe_allow_html=True)
    st.markdown("---")
    ...
    for _,r in df[df["score"]<0].sort_values("score").head(15).iterrows():
        ...
        smf_txt = f' | SMF: {smf_icon}{r.get("smf_value",0):+.2f}' if r.get("smf_value",0) != 0 else ""
        div_txt = " ⚡DIV" if r.get("smf_diverging") else ""
        st.markdown(f'<div style="background:#1a1a2e;border-radius:8px;padding:10px 14px;margin:4px 0;"><div style="display:flex;justify-content:space-between;"><b style="color:white;">{r["symbol"]}</b><span style="color:#FF6347;font-weight:bold;">{r.get("confluence_rec","WAIT")} ({r["score"]})</span></div><div style="background:#2a2a4a;border-radius:4px;height:6px;margin-top:6px;"><div style="background:linear-gradient(90deg,#FF6347,#FF0000);height:6px;width:{min(s,100)}%;border-radius:4px;"></div></div><div style="font-size:11px;color:#888;margin-top:4px;">RSI 4h: {r.get("rsi_4h",50):.1f} | MACD: {r.get("macd_trend","—")} | Vol: {r.get("vol_trend","—")}{smf_txt}{div_txt}</div></div>', unsafe_allow_html=True)
...
**Smart Money Flow (SMF):** Misst WO der Preis innerhalb jeder Kerze schließt, gewichtet mit Volumen. CLV×Volume zeigt ob Smart Money akkumuliert (+) oder distribuiert (-). Werte > +0.4 = starke Akkumulation, < -0.4 = starke Distribution. ⚡DIV = Smart Money handelt gegen den Preistrend (potentielle Trendwende).

**Ideales Kauf-Setup:** RSI < 42 + EMA bullish + MACD bullish + Bollinger < 20% + SMF Akkumulation → starkes Kaufsignal mit hoher Erfolgswahrscheinlichkeit.

**Ideales Verkauf-Setup:** RSI > 58 + EMA bearish + MACD bearish + Bollinger > 80% + SMF Distribution → starkes Verkaufsignal.""")
